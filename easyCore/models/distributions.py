...
class Gaussian(BaseObj):
    ...
    def __init__(self, amplitude, mean, cov_matrix, name=None):
        ...
        super(Gaussian, self).__init__(name, amplitude=amplitude, mean=mean, cov_matrix=cov_matrix)

    @classmethod
    def from_pars(cls, amplitude, mean, cov_matrix):
        amplitude = Parameter('amplitude', amplitude)
        if isinstance(mean, Iterable):
            mean = BaseCollection('mean', *[Parameter(f'mean_{ind}', mean_i) for ind, mean_i in enumerate(mean)])
        else:
            mean = Parameter('mean', mean)
        if isinstance(cov_matrix, Iterable):
            cov_matrix = np.array(cov_matrix)
            cov_matrix = BaseCollection('cov_matrix', *[
                Parameter(f"cov_{'_'.join([str(i) for  i in idx])}", cov_matrix[idx])
