...
class Polynomial(PolynomialBase):
    ...
    def __call__(self, x: Union[jnp.ndarray, DeviceArray], *args, **kwargs) -> DeviceArray:
        v = jnp.array([c.raw_value for c in self.coefficients])
        return self._calculate(x, v)

    @staticmethod
    @jit
    def _calculate(x: Union[jnp.ndarray, DeviceArray], values: DeviceArray) -> DeviceArray:
        return jnp.polyval(values, x)

    @jit
    def model(self, theta: Union[List, Tuple, jnp.ndarray, DeviceArray], x: Union[jnp.ndarray, DeviceArray]):
...
class Line(LineBase):
    ...
    @staticmethod
    @jit
    def _calculate(x: Union[jnp.ndarray, DeviceArray], m: float, c: float) -> DeviceArray:
        return m * x + c
