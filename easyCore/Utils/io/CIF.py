...
from easyCore.Utils.io.star import StarCollection, StarEntry, StarLoop, FakeItem, FakeCore
...
class CifParser:
    ...
    def get_lattice(self, length_strings=("a", "b", "c"),
        ...
        """
        data = self._cif['data']
        find_keys = ['_cell_length_' + key for key in length_strings]
        find_keys.extend(['_cell_angle_' + key for key in angle_strings])
        ...
        try:
            ...
            for idx, key in enumerate(find_keys):
                data_dict[dict_keys[idx]] = data[key].raw_value
                if lattice_type is None:
            ...
            for idx, key in enumerate(find_keys):
                ...
                if hasattr(data[key], 'fixed'):
                    obj.fixed = data[key].fixed
                if hasattr(data[key], 'error'):
                    obj.error = data[key].error
            return lattice
        except KeyError:
