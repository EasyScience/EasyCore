...
class Anisotropy(BaseObj):
    ...
    def __init__(self, adp_type: Descriptor, interface=None, **kwargs):
        ...
        super(Anisotropy, self).__init__('anisotropy', adp_type=adp_type, adp_class=adp_class)
    ...
    @classmethod
    def from_string(cls, in_string: str) -> Tuple[List[str], List['Anisotropy']]:
        ...
        for section in sections:
            ...
            for idx, par in enumerate(pars):
                ...
                par_dict[adp_class._CIF_CONVERSIONS[idx][0]] = entries[idx_list[-1]].value
            obj = cls.from_pars(adp_type, **par_dict)
            for idx in idx_list:
                if hasattr(entries[idx], 'fixed'):
                    if entries[idx].fixed is not None:
                        entry = getattr(obj, list(par_dict.keys())[idx])
                        entry.fixed = entries[idx].fixed
                if hasattr(entries[idx], 'error'):
                    if entries[idx].error is not None:
                        entry = getattr(obj, list(par_dict.keys())[idx])
    ...
    def available_types(self) -> List[str]:
        return [name[0] for name in _AVAILABLE_ISO_TYPES]
...
class Isotropic(AdpBase):
    ...
    def __init__(self, Uiso: Parameter, interface=None):
        super(Isotropic, self).__init__(Uiso=Uiso)
        self.interface = interface
        if self.interface is not None:
            self.interface.generate_bindings(self)
...
class IsotropicB(AdpBase):
    ...
    def __init__(self, Biso: Parameter, interface=None):
        super(IsotropicB, self).__init__(Biso=Biso)
        self.interface = interface
        if self.interface is not None:
            self.interface.generate_bindings(self)
