...
def test_basic_fit_2D_xarrays(genObjs):
    ref_sin = genObjs[0]
    sp_sin = genObjs[1]
    ...
    y = np.linspace(x_min, x_max, num=int(nx)) + np.pi
    d.easyCore.add_coordinate("x", x)
    d.easyCore.add_coordinate("y", y)
    coordinates = ["x", "y"]
    coords = [d[da] for da in coordinates]
    points = xr.broadcast(*coords)
    Z = ref_sin(*points)

    d.easyCore.add_variable("z", ["x", "y"], Z.data)

    f = Fitter(sp_sin, sp_sin)

    _ = f.fit({point.name: point for point in points}, d["z"])
...
#     assert result.goodness_of_fit == pytest.approx(0, abs=1.5e-5)
#     assert result.reduced_chi == pytest.approx(0, abs=1.5e-5)
...
#         assert item1.raw_value == pytest.approx(item2.raw_value, abs=1e-3)
...
#     sp_sin.phase.fixed = False
#
#     sp_ref1 = {f'p{sp_sin._borg.map.convert_id_to_key(item1)}': item1.raw_value for item1, item2 in zip(sp_sin._kwargs.values(), ref_sin._kwargs.values())}
#     sp_ref2 = {f'p{sp_sin._borg.map.convert_id_to_key(item1)}': item2.raw_value for item1, item2 in zip(sp_sin._kwargs.values(), ref_sin._kwargs.values())}
#
#     f = Fitter(sp_sin, sp_sin)
#
#     if fit_engine is not None:
