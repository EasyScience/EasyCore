...
class COS(BaseObj):
    def __init__(self, name, amplitude, phase, period, offset):
        super(COS, self).__init__(name, amplitude=amplitude, phase=phase, period=period, offset=offset)

    def __call__(self, x):
        return self._calculate((self.amplitude.raw_value,
                                self.phase.raw_value,
                                self.period.raw_value,
                                self.offset.raw_value), x)
    ...
    def from_pars(cls, amp, phase, period, offset):
        return cls('cosine', PrimalValue('amplitude', amp), PrimalValue('phase', phase), PrimalValue('period', period),PrimalValue('offset', offset))

    @classmethod
    def default(cls):
        return cls.from_pars(1., 0., 1., 1.)
...
def update(theta, x, y, LEARNING_RATE=1e-3):
    ...
    tH = jax.tree_multimap(
      lambda param, g: param - g * LEARNING_RATE, theta, grad)
    return tH
...
theta = np.array([cos2.amplitude.value, cos2.phase.value, cos2.period.value, cos2.offset.value])
Jacobian = jax.jit(lambda xx, th: jax.vmap(lambda _x: jax.jacfwd(lambda _theta: cos2.model(_theta, _x))(th))(xx))
