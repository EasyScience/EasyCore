...
d.easyCore.add_variable(
    "z",
    ["x", "y"],
    np.sin(2 * np.pi * (x + sin_offest)).reshape((-1, 1))
    * np.cos(2 * np.pi * (x + cos_offset)).reshape((1, -1))
...
t = time.time()
f_res = d["z"].easyCore.fit(f, vectorize=True)
...
d["z"].plot(ax=axes[0])
d["computed_no_dask"].plot(ax=axes[1])
temp.plot(ax=axes[2])
plt.tight_layout()
plt.show()
