...
class Wavey(BaseObj):
    def __init__(self, amplitude: Parameter, phase: Parameter, period: Parameter):
        super(Wavey, self).__init__(
            "SimpleWave", amplitude=amplitude, phase=phase, period=period
        )

    def __repr__(self):
        return (
            f"{self.name} - A={self.amplitude}, ph={self.phase}, period={self.period}"
        )

    @classmethod
    def from_params(
        cls, amplitude: float = 1, phase: float = 0, period: float = 2 * np.pi
    ):
...
y1 = bb.fit_fun(x) + 0.75 * (0.5 - np.random.random(size=x.shape))
x2 = x + 20
y2 = bb.fit_fun(x2) + 0.75 * (0.5 - np.random.random(size=x2.shape))
